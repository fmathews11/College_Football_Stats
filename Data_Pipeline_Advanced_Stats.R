...
ncaaf_l1 <- read.csv('Level_One_With_ELO.csv',stringsAsFactors = TRUE)

ncaaf_l1 <- ncaaf_l1%>%
  mutate(Home = ifelse(Home == 0,FALSE,TRUE))
...
for(i in 1:length(unique(ncaaf_l1$Game_ID))){
  ...
  # if(!is.null(nrow(ncaaf_l1$Neutral_Location[ncaaf_l1$Game_ID == game_id & ncaaf_l1$Home == TRUE])) & nrow(ncaaf_l1$Neutral_Location[ncaaf_l1$Game_ID == game_id & ncaaf_l1$Home == TRUE]) == 2){
  ...
  if(update_data){
    ...
    Interceptions_Thrown_Home <- game_stats[game_stats$Stat == "Interceptions thrown", 3][0] %>% as.numeric()
    Interceptions_Thrown_Away <- game_stats[game_stats$Stat == "Interceptions thrown", 2][0] %>% as.numeric()
